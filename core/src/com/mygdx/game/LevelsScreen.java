...
public class LevelsScreen extends ScreenAdapter {
    ...
    float rectHeight;
    float moveSpeed = 600;
    private LEVELS lastCompletedLevel;

    LevelsScreen(Foresty game) {
        this.game = game;
        lastCompletedLevel = LEVELS.ZERO;
    }
    ...
    public void show() {
        ...
        final HashMap<AnimalsTypes, Integer> levelFirstAnimals = new HashMap<>();
        //levelFirstAnimals.put(AnimalsTypes.RABBIT, 1);
        //levelFirstAnimals.put(AnimalsTypes.HORSE, 1);
        levelFirstAnimals.put(AnimalsTypes.GOAT_BABY, 1);
        levelFirstAnimals.put(AnimalsTypes.GOAT, 1);
        ...
        Gdx.input.setInputProcessor(new InputAdapter() {
            ...
            public boolean touchDown(int screenX, int screenY, int pointer, int button) {
                ...
                if (screenX >= map.getWidth() / 10 - firstLevel.getWidth() / 2
                    ...
                    game.setScreen(new GameScreen(game, levelFirstAnimals, 15, 30, 45, 75, LEVELS.ONE));
                }
    ...
    public void render(float data) {
        ...
        for (int i = 0; i < LEVELS.ONE.getNumOfStars(); i++) {
        ...
        if (lastCompletedLevel.getNum() + 1 < 2) {
        ...
        for (int i = 0; i < LEVELS.TWO.getNumOfStars(); i++) {
        ...
        if (lastCompletedLevel.getNum() + 1 < 3)
        ...
        for (int i = 0; i < LEVELS.THREE.getNumOfStars(); i++) {
        ...
        if (lastCompletedLevel.getNum() + 1 < 4)
        ...
        for (int i = 0; i < LEVELS.FOUR.getNumOfStars(); i++) {
        ...
        if (lastCompletedLevel.getNum() + 1 < 5)
        ...
        for (int i = 0; i < LEVELS.FIVE.getNumOfStars(); i++) {
    ...
    public void levelCompleted() {
        if (lastCompletedLevel.getNum() == 0)
            game.levelsScreen.setLastCompletedLevel(LEVELS.ONE);
        else if (lastCompletedLevel.getNum() == 1)
            game.levelsScreen.setLastCompletedLevel(LEVELS.TWO);
        else if (lastCompletedLevel.getNum() == 2)
            game.levelsScreen.setLastCompletedLevel(LEVELS.THREE);
        else if (lastCompletedLevel.getNum() == 3)
            game.levelsScreen.setLastCompletedLevel(LEVELS.FOUR);
        else if (lastCompletedLevel.getNum() == 4)
            game.levelsScreen.setLastCompletedLevel(LEVELS.FIVE);
    }
    ...
    public LEVELS getLastCompletedLevel() {
        return lastCompletedLevel;
    }

    public void setLastCompletedLevel(LEVELS lastCompletedLevel) {
